  ...
  methods: {
    ...
    getMaterial () {
      this.$axios({
        url: '/user/images', // 请求地址
        params: {
        //  这个位置应该变活 根据当前的页签变活 activeName === 'all' 获取所有的素材 activeName = 'collect' 获取收藏素材
          collect: this.activeName === 'collect',
          // 取页码变量中的值 因为只要页码变量一变 获取的数据跟着变
        ...
        // 将总条数赋值给total变量
        // 总数  全部素材的总数  收藏素材的总数 总数 跟随 当前页签变化而变化
        this.page.total = result.data.total_count
      })
    },
    // 切换页签事件
    changeTab () {
    // 将页码重置为第一页 因为分类变了 数据变了
